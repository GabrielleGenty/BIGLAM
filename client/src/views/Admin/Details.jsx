...
function Details() {
  ...
  const [showSuccessMessageOnly, setShowSuccessMessageOnly] = useState(false); // Nouvel état
  const { id } = useParams();
  ...
  async function updateProductHandler(e) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setShowSuccessMessageOnly(true); // Masquer les détails et afficher uniquement le message de succès

        // Afficher le message de succès pendant quelques secondes
        setTimeout(() => {
          setSuccessMessage("");
          navigate("/products"); // Redirect to products page after a short delay
        }, 2000); // Délai prolongé pour que l'utilisateur puisse lire le message
      } else {
